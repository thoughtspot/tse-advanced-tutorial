<script>
  ...
  const tsInitialize = async () => {
    ...
    // TODO - add an init block from the playgorund to initial ThoughtSpot using trusted authentication.
    const ee = null;
  ...
  const setVersion = () => {
    ...
    // TODO add code to get the version and assign it to $store.version
  }
  ...
  const toggleCSS = () => {
    ...
    // TODO call initialize to update the CSS.
  }
